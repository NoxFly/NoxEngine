...
namespace NoxEngine {
    ...
    bool Actor<D>::load() {
        ...
        const unsigned int nC = m_material.getColors().size();
        // number of textures (Texture*)
        const unsigned int nTex = m_material.getTextures().size();
        // material texture's number
        const unsigned int mnT = m_material.getTextures().size();
        ...
        const unsigned int nT = mnT > 0 ? m_geometry.getTextures().size() : 0;
        ...
        if(nC > 0) {
            ...
            for(size_t i=0; i < nC; i++) {
                ...
                while(++k <= m_vertexNumber && ++j <= d) {
                    ...
                    colors.push_back((float)c.r / 255.0f);
                    colors.push_back((float)c.g / 255.0f);
                    colors.push_back((float)c.b / 255.0f);
                    colors.push_back((float)c.a / 255.0f);
                }
            }
        }
        ...
        const unsigned int vSize = fsize * nV;
        const unsigned int cSize = fsize * colors.size(); // cause size is number of Color, which's composed by r,g,b[,a]
        const unsigned int tSize = fsize * nT;
