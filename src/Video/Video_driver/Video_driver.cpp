...
bool Video_driver::loadLastGlVersion() {
    ...
    if(!Video_driver::video.hasWindow()) {
        std::cout << "[Video_driver::loadLastGlVersion Error] : Cannot load openGL version" << std::endl;
        return false;
    }
    ...
    try {
        ...
        std::cout << "[Video_driver::loadLastGlVersion Error] : Cannot recover GL_VERSION :\n" << e.what() << std::endl;
    ...
    if(v[0] > 0 && v[1] > 0) {
        ...
        std::cout << "[Video_driver::loadLastGlVersion] Load OpenGL " << v[0] << "." << v[1] << std::endl;
