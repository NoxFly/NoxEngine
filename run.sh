...
compile()
{
    make ${mode^^}=1 PGNAME=$pgname SRCEXT=$fileExt SRCDIR=$srcDir INCDIR=$includeDir OUT=$outDir BUILDDIR=$buildDir MODE=$projectMode CVERSION=$cVersion CPPVERSION=$cppVersion
}
